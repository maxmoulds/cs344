...
int otp_enc_d(int port) {
  ...
  *id = 10; 
  ...
  while(1) {
    ...
    if ((*id) == OTP_ENC_KEYFILE_INT)
    {
      //read into keybuf.?
      if (keybuf_chars == 0) 
      {
        //first time
        memcpy(keybuf, buffer, MAX_READ);
        memcpy(ciphertext, buffer, MAX_READ);
        //*id = OTP_ENC_D_KEYFILE_RESPONSE_INT;
        //cleanup
      }
      else
      {
        //extend the keybuf...
        keybuf = realloc(keybuf, strlen(buffer)+1);
        strcat(keybuf, buffer);//eww, maybe strncat...
        memcpy(ciphertext, buffer, MAX_READ);
      }
      *id = OTP_ENC_D_KEYFILE_RESPONSE_INT;
    }
    else if ((*id) != OTP_ENC_INT) 
    {
